...
_sfc_main.styles = `body {  font-family: monospace; background: #0c0c0e; color: #e1e1e6; padding: 2rem;  }  .tester-root[data-h-tester] {  background: #141416; padding: 1.5rem; border-radius: 8px; border: 1px solid #232326;  }  .controls[data-h-tester] {  display: flex; gap: 10px; margin-bottom: 20px; flex-wrap: wrap;  }  button[data-h-tester] {  background: #04d361; color: #000; border: none; padding: 10px 15px; font-weight: bold; cursor: pointer; border-radius: 4px;  }  button.danger[data-h-tester] {  background: #ff4444; color: white;  }  .grid[data-h-tester] {  display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 10px; height: 50vh; overflow-y: auto; padding: 10px; background: #000; border: 1px solid #333;  }  .row[data-h-tester] {  background: #1c1c1f; padding: 10px; border-radius: 4px; display: flex; align-items: center; justify-content: space-between; border-left: 3px solid #04d361;  }  .id-badge[data-h-tester] {  color: #888; font-size: 0.8em;  }`;
_sfc_main.render = function(context, instanceId) {
    ...
    if (context.dataList !== undefined) {
        registerListSink(root.childNodes[2].childNodes[0], context.dataList, 'id', 'item', instanceId, compileDirectives);
    }
