...
const _sfc_main = {
  ...
  computed: {
    panelClasses(ctx) {
      return {
        'dark-mode': ctx.darkMode,
        'light-mode': !ctx.darkMode
      };
    },
    previewStyles(ctx) {
      return {
    ...
    modeLabel(ctx) {
      return ctx.darkMode ? '🌙 Oscuro' : '☀️ Claro';
    }
  },
  methods: {
    toggleDarkMode(ctx) {
...
_sfc_main.render = function(context, instanceId) {
    ...
    {
        const targetEl = root.childNodes[2].childNodes[0].childNodes[0];
        if (context.previewMessage !== undefined) {
            registerSink(targetEl, SINK_TYPE_TEXT, context.previewMessage, instanceId);
        }
    }
    {
        const targetEl = root.childNodes[1].childNodes[0].childNodes[1].childNodes[0].childNodes[0];
